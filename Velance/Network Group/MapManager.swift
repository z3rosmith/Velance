...
class MapManager {
    ...
    func searchByKeyword(
        with model: SearchMallDTO,
        completion: @escaping ((Result<SearchMallByKeywordDTO,NetworkError>) -> Void)
    ) {
        AF.request(
            ...
            case 200:
                do {
                    let decodedData = try JSONDecoder().decode(SearchMallByKeywordDTO.self,
                                                               from: response.data!)
